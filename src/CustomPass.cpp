...
void Registry::DrawDebugOverlay() {
    std::lock_guard lk(mutex);
    ImGui::SetNextWindowPos(ImVec2(580, 320), ImGuiCond_FirstUseEver);
    ...
    if (ImGui::CollapsingHeader("Passes", ImGuiTreeNodeFlags_DefaultOpen)) {
        if (ImGui::BeginTable("passes_tbl", 6, kTableFlags)) {
            ...
            for (auto& p : passes) {
                ...
                if (lf == UINT32_MAX) {
                    ImGui::TextColored(ImVec4(0.6f, 0.6f, 0.6f, 1), "never");
                } else {
                    const uint32_t age = currentFrame - lf;
                    if (age <= 1) {
                        ImGui::TextColored(ImVec4(0.4f, 0.95f, 0.4f, 1), "this frame");
                    } else if (age < 10000) {
                        ImGui::TextColored(ImVec4(0.85f, 0.85f, 0.4f, 1), "%u frames ago", age);
                    } else {
                        ImGui::TextColored(ImVec4(0.6f, 0.6f, 0.6f, 1), "stale");
                    }
                }
    ...
    if (ImGui::CollapsingHeader("Resources", ImGuiTreeNodeFlags_DefaultOpen)) {
        if (ImGui::BeginTable("resources_tbl", 5, kTableFlags)) {
            ...
            for (auto& r : resources) {
                ...
                ImGui::TextColored(ok ? ImVec4(0.4f, 0.95f, 0.4f, 1) : ImVec4(0.95f, 0.3f, 0.3f, 1),
                                   "%s", ok ? "yes" : "NO");
